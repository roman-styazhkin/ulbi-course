...
describe('classNames', () => {
    ...
    test('with mods', () => {
        ...
        expect(classNames('mainClass', {selected: true, scrollable: true}, ['class1', 'class2'])).toBe(expected);
    });

    test('with mods with false', () => {
        const expected = 'mainClass class1 class2 selected';
        expect(classNames('mainClass', {selected: true, scrollable: false}, ['class1', 'class2'])).toBe(expected);
    });

    test('with mods with undefined', () => {
        const expected = 'mainClass class1 class2 selected';
        expect(classNames('mainClass', {selected: true, scrollable: undefined}, ['class1', 'class2'])).toBe(expected);
    });
});
